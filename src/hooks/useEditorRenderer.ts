...
function toScreen(fx: number, fy: number, vt: ViewTransform): [number, number] {
  return [
  ...
  for (const path of paths) {
    ...
    for (const cmd of path.commands) {
      if (cmd.kind === 'M') {
        ...
        const tangentX =
          3 * (mt2 * (p1.x - p0.x) + 2 * mt * t * (p2.x - p1.x) + t2 * (p3.x - p2.x));
        const tangentY =
          3 * (mt2 * (p1.y - p0.y) + 2 * mt * t * (p2.y - p1.y) + t2 * (p3.y - p2.y));
        const [sx, sy] = toScreen(midX, midY, vt);
        const [tx, ty] = toScreen(midX + tangentX, midY + tangentY, vt);
        const dx = tx - sx;
        const dy = ty - sy;
...
export function renderFrame(
  ...
  if (showPixelGrid) {
    ...
    for (let fx = startFX; fx <= visMaxFX + gridInterval; fx += gridInterval) {
      ...
      skCanvas.drawLine(
        sx,
        Math.max(RULER_WIDTH, gridTopSy),
        sx,
        Math.min(canvasHeight, gridBotSy),
        gridPaint
      );
    }
    ...
    for (let fy = startFY; fy <= visMaxFY + gridInterval; fy += gridInterval) {
      ...
      skCanvas.drawLine(
        Math.max(RULER_WIDTH, gridLeftSx),
        sy,
        Math.min(canvasWidth, gridRightSx),
        sy,
        gridPaint
      );
    }
    ...
    for (const path of paths) {
      for (const cmd of path.commands) {
        const drawOnCurve = (pt: { id: string; x: number; y: number }) => {
          ...
          spaint.setColor(
            isSelected ? C.selectedStroke : isHovered ? C.hoveredStroke : C.onCurveStroke
          );
        ...
        const drawOffCurve = (pt: { id: string; x: number; y: number }) => {
          ...
          spaint.setStrokeWidth(1.5);
          spaint.setColor(
            isSelected ? C.selectedStroke : isHovered ? C.hoveredStroke : C.offCurveStroke
          );
        ...
        if (cmd.kind === 'M' || cmd.kind === 'L') drawOnCurve(cmd.point);
          ...
          drawOffCurve(cmd.ctrl);
          drawOnCurve(cmd.point);
        } else if (cmd.kind === 'C') {
          drawOffCurve(cmd.ctrl1);
          drawOffCurve(cmd.ctrl2);
          drawOnCurve(cmd.point);
        }
      }
    }
  ...
  if (showTransformBox && selection.pointIds.size > 1) {
    ...
    for (const path of paths) {
      for (const cmd of path.commands) {
        const pts =
          cmd.kind === 'M' || cmd.kind === 'L'
            ? [cmd.point]
            : cmd.kind === 'Q'
              ? [cmd.ctrl, cmd.point]
              : cmd.kind === 'C'
                ? [cmd.ctrl1, cmd.ctrl2, cmd.point]
                : [];
        for (const pt of pts) {
